...
- Pitch exists at `outside-design/docs/pitches/<name>.md` (or user provides path)
...
1. **Identify pitch and derive slug**
   - Confirm which pitch is being promoted (path under `outside-design/docs/pitches/<name>.md` or user-specified).
...
2. **Apply create-delivery-branch**
   - Read and follow [create-delivery-branch](.cursor/skills/create-delivery-branch/SKILL.md) with branch name `feature/<delivery-slug>`.

3. **Apply create-delivery-folder**
   - Read and follow [create-delivery-folder](.cursor/skills/create-delivery-folder/SKILL.md) with delivery slug and current date-time.
   - Result: folder `outside-design/docs/deliveries/{YYYY-MM-DD-HHMM}-{delivery-slug}/` with placeholder files.

4. **Apply move-pitch-to-delivery**
   - Read and follow [move-pitch-to-delivery](.cursor/skills/move-pitch-to-delivery/SKILL.md).
   - Inputs: source = pitch path from step 1; destination = delivery folder from step 3.

5. **Apply create-implementation-plan**
   - Read and follow [create-implementation-plan](.cursor/skills/create-implementation-plan/SKILL.md).
...
6. **Apply create-delivery-roadmap**
   - Read and follow [create-delivery-roadmap](.cursor/skills/create-delivery-roadmap/SKILL.md) with delivery folder path and plan at `{delivery-folder}/plan.md`.

7. **Apply create-delivery-readme**
   - Read and follow [create-delivery-readme](.cursor/skills/create-delivery-readme/SKILL.md) with delivery folder path, delivery title (from pitch), branch `feature/<delivery-slug>`, and optional one-sentence summary (from pitch).
