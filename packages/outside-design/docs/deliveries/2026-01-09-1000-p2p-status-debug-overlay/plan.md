...
Modified `[packages/outside-client/src/network/webrtc.ts](packages/outside-client/src/network/webrtc.ts)` to expose a connection state change handler.
...
Modified `[packages/outside-client/src/debug/overlay.ts](packages/outside-client/src/debug/overlay.ts)` to include a P2P status display.
...
- In `[packages/outside-client/src/network/client.ts](packages/outside-client/src/network/client.ts)`:
  - Added `onConnectionStateChange` to `ClientCallbacks`.
  - Set up the handler on the `hostPeer` in `initiateConnection`.

### 4. Wire Up in Main

Updated `[packages/outside-client/src/main.ts](packages/outside-client/src/main.ts)` to connect the network callbacks to the debug overlay.
