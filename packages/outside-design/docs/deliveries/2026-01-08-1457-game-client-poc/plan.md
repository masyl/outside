...
- `packages/outside-client/package.json` - Added dependencies (pixi.js, immer, motion, vite)
- `packages/outside-client/vite.config.ts` - Vite configuration
- `packages/outside-client/tsconfig.json` - TypeScript config
- `packages/outside-client/index.html` - HTML entry point
- `packages/outside-client/src/main.ts` - Application entry point
...
- `packages/outside-core/src/types.ts` - Shared types (Grid, Position, Object, Bot, etc.)
- `packages/outside-core/src/world.ts` - World state model (20x10 grid)
...
- `packages/outside-client/src/store/store.ts` - Flux store with Immer
- `packages/outside-client/src/store/actions.ts` - Action creators
- `packages/outside-client/src/store/reducers.ts` - State reducers using Immer
...
- `packages/outside-client/src/commands/parser.ts` - Custom browser-compatible parser
- `packages/outside-client/src/commands/handlers.ts` - Command handlers (create, place, move)
- `packages/outside-client/src/commands/queue.ts` - Command queue for game loop
...
- `packages/outside-client/src/renderer/renderer.ts` - Pixi.js setup and main renderer
- `packages/outside-client/src/renderer/grid.ts` - Grid rendering (checkered background)
- `packages/outside-client/src/renderer/objects.ts` - Object rendering (bot sprites)
...
- `packages/outside-client/src/game/loop.ts` - Game loop manager
- `packages/outside-client/src/game/animations.ts` - Animation system (initial implementation)
...
- `packages/outside-client/src/mock/commandFeeder.ts` - Programmatic command feeder
...
- `packages/outside-client/src/main.ts` - Wired everything together:
...
- `packages/outside-client/src/debug/overlay.ts` - Debug overlay with FPS counter, step counter, and version
