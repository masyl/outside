...
- Updated `createTerrainLayer` and `createTerrainSprite` in `packages/outside-client/src/renderer/terrain.ts`.
...
- Updated `createBotSprite` and `updateObjectsLayerWithIndex` in `packages/outside-client/src/renderer/objects.ts`.
