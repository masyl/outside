...
- **Dependency**: `wavefunctioncollapse` (v2.1.0) added to outside-storybook; type declarations in `packages/outside-storybook/src/types/wavefunctioncollapse.d.ts`.
- **WFC dungeon module**: `packages/outside-storybook/src/utils/dungeonLayoutWFC.ts` — minimal floor/wall tileset, seeded RNG, `generateDungeonWFC(width, height, seed)` returning `DungeonResult`; up to 3 retries on contradiction, then fallback to `generateDungeon`.
