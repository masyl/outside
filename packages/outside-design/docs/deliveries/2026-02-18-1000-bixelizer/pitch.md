...
> **This pitch was modified during implementation.**
> See [Amendments](#amendments) for a summary of scope changes.
...
- ~~Storybook story in `packages/outside-storybook/` displaying each generated font's sprite grid at 4× scale, grouped by Unicode block, one story per font~~ *(superseded — see Storybook Browser below)*
- **Storybook Font Browser** — a single interactive story covering all generated fonts, with:
  - Full sprite grid for each font displayed in one continuous list (not split by charset)
  - Storybook control to filter by **font** (individual font or "All fonts")
  - Storybook control to filter by **charset** (cp437, plus, or "All sets")
  - Storybook control to select a **display mode**: full sprite grid, or a demo text phrase
  - Demo text phrases include:
    - Common pangrams (e.g. "The quick brown fox…")
    - Famous short quotes and poem excerpts
    - Longer multi-paragraph texts
    - ASCII art samples: box-drawing characters, block shading fades, classic emoticons
  - Mouse-over tooltip overlay showing: font name, foundry, glyph dimensions, Unicode code point, Unicode name, CP437 index (if applicable)
  - Glyphs displayed at 4× scale with pixelated rendering
...
Added after initial implementation was underway:

1. **Storybook Font Browser** (replaces the original "one story per font" approach): the Storybook story is now a unified multi-font browser with filtering controls and demo text modes. The old approach (grouped by Unicode block, one story per font) is struck through in Inclusions.

2. **Three additional fonts processed**: Robotron A7100 Square, Nix8810 M16, FMTowns re. 8×16 — all at 8×16 px. The renderer was updated to support non-square glyph cells (`glyphWidth` / `glyphHeight`).
