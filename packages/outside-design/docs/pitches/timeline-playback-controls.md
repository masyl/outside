...
Extend `GameLoop` class with pause/resume/step methods that interact with the Timeline Manager. Create `PlaybackState` enum in `packages/outside-client/src/timeline/playbackState.ts` with values: PLAYING, PAUSED, TRAVELING.
