...
Create `TimelineBar` class in `packages/outside-client/src/ui/timelineBar.ts` that extends PIXI `Container`. The bar will use PIXI `Graphics` for the green background and a smaller `Graphics` rectangle for the position indicator. Position will be calculated as `(currentStep / totalSteps) * barWidth`.

Extend `DebugOverlay` in `packages/outside-client/src/debug/overlay.ts` to add timeline status fields. Add methods `setPlaybackState(state)` and `setTimelinePosition(currentStep, totalSteps)` to update the display.
