...
interface TestPlayerCanvasProps {
...
let NEXT_TEST_PLAYER_PIXI_APP_ID = 1;

export const TestPlayerCanvas: React.FC<TestPlayerCanvasProps> = ({
  children,
  width = '100%',
  height = '100%',
  cursor = 'default',
  ...
  useEffect(() => {
    ...
    const appId = NEXT_TEST_PLAYER_PIXI_APP_ID++;
    appRef.current = app;
    ...
    const destroyApp = () => {
      ...
      try {
        ...
        console.warn('[TestPlayerCanvas] destroy failed', error);
      }
    };
    ...
    (async () => {
      try {
        ...
        app.stage.label = `test-player:pixi-app#${appId}:stage`;
        readyRef.current = true;
        setReadyVersion((v) => v + 1);
        childrenRef.current(app);
      } catch (error) {
        console.warn('[TestPlayerCanvas] init failed', error);
        destroyApp();
      }
    })();
  ...
  useEffect(() => {
    ...
    const resizeToContainer = () => {
      ...
      try {
        ...
        console.warn('[TestPlayerCanvas] resize failed', error);
      }
    };
