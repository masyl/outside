...
Extend `KeyboardHandler` in `outside-client/src/input/keyboardHandler.ts` to add timeline keystrokes. Each handler will check for the modifier key (event.altKey) before executing timeline actions.
