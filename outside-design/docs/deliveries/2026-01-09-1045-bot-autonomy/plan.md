...
- Updated `WorldState` in `[outside-core/src/types.ts](outside-core/src/types.ts)` to include a `seed: number`.
- Updated `createWorldState` to generate a random master seed.
- Created `[outside-client/src/game/random.ts](outside-client/src/game/random.ts)` with a `Random` class (LCG).

### 3. Autonomous Bot Logic

Created a new system for handling bot autonomy.

- Created `[outside-client/src/game/autonomy.ts](outside-client/src/game/autonomy.ts)`.
