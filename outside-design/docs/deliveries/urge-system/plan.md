...
- Commands are parsed in `outside-client/src/commands/parser.ts` and executed in `outside-client/src/commands/handlers.ts`.
- Motion is advanced deterministically via `SIM_TICK` in `outside-client/src/store/reducers.ts` (20Hz fixed-step from host loop).
- Demo level bot spawns come from `outside-client/public/levels/demo.md`.
...
- Extend `ParsedCommand` in `outside-client/src/commands/parser.ts`
- Handle the new command types in `outside-client/src/commands/handlers.ts`
- Add store actions (e.g. `SET_BOT_URGE`) in `outside-client/src/store/actions.ts`
- Update reducer to apply urge changes to the bot entity in `outside-client/src/store/reducers.ts`
...
Update `outside-client/public/levels/demo.md`:
