...
describe('computeWalkFrameIndex', () => {
  ...
  it('cycles once per tile traveled', () => {
    // speed = 1 tile/sec, frames = 4 → 4 fps
    ...
    expect(computeWalkFrameIndex({ timeMs: 500, speedTilesPerSec: 1, frames: 4 })).toBe(2);
    expect(computeWalkFrameIndex({ timeMs: 750, speedTilesPerSec: 1, frames: 4 })).toBe(3);
    expect(computeWalkFrameIndex({ timeMs: 1000, speedTilesPerSec: 1, frames: 4 })).toBe(0);
  });

  it('runs faster when speed increases', () => {
    // speed = 2 tiles/sec, frames = 4 → 8 fps
    expect(computeWalkFrameIndex({ timeMs: 125, speedTilesPerSec: 2, frames: 4 })).toBe(1);
    expect(computeWalkFrameIndex({ timeMs: 250, speedTilesPerSec: 2, frames: 4 })).toBe(2);
  });
});
